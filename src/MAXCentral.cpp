...
void MAXCentral::stopThreads() {
  try {
    ...
    {
      std::lock_guard<std::mutex> pairingModeThreadGuard(_pairingModeThreadMutex);
...
bool MAXCentral::isSwitch(uint32_t type) {
  switch ((DeviceType)type) {
    case DeviceType::BCTSSWPL: return true;
...
bool MAXCentral::onPacketReceived(std::string &senderID, std::shared_ptr<BaseLib::Systems::Packet> packet) {
  try {
    ...
    if (GD::bl->debugLevel >= 4)
      std::cout << BaseLib::HelperFunctions::getTimeString(maxPacket->getTimeReceived())
                << " MAX packet received (" + senderID + (maxPacket->rssiDevice() ? ", RSSI: 0x" + BaseLib::HelperFunctions::getHexString(maxPacket->rssiDevice(), 2) : "") + "): " + maxPacket->hexString() << std::endl;
    ...
    if (_pairing) {
      std::lock_guard<std::mutex> pairingInterfaceGuard(_pairingInterfaceMutex);
      if (!_pairingInterface.empty() && senderID != _pairingInterface) return false;
    }
  ...
  try {
    ...
    for (BaseLib::Database::DataTable::iterator row = rows->begin(); row != rows->end(); ++row) {
      ...
      switch (row->second.at(2)->intValue) {
        case 1: _centralAddress = row->second.at(3)->intValue;
          break;
        case 2: unserializeMessageCounters(row->second.at(5)->binaryValue);
          break;
      }
    }
  }
      ...
      try {
        ...
        for (std::map<uint64_t, std::shared_ptr<BaseLib::Systems::Peer>>::iterator i = _peersById.begin(); i != _peersById.end(); ++i) {
          ...
          if (i->second->getFirmwareVersion() == 0) stringStream << std::setfill(' ') << std::setw(firmwareWidth) << "?" << bar;
            ...
            stringStream << std::setfill(' ') << std::setw(firmwareWidth) << ("*" + BaseLib::HelperFunctions::getHexString(i->second->getFirmwareVersion() >> 4) + "." + BaseLib::HelperFunctions::getHexString(i->second->getFirmwareVersion() & 0x0F))
                         << bar;
          } else
  ...
  try {
    ...
    if (_pairing) {
      ...
      if (!peer) {
        ...
        if (!queue->peer) {
          GD::out.printWarning("Warning: Device type 0x" + GD::bl->hf.getHexString(deviceType, 4) + " with firmware version 0x" + BaseLib::HelperFunctions::getHexString(firmwareVersion, 4) + " not supported. Sender address 0x"
                                   + BaseLib::HelperFunctions::getHexString(packet->senderAddress(), 6) + ".");
          return;
        }
        peer = queue->peer;

        std::lock_guard<std::mutex> pairingInterfaceGuard(_pairingInterfaceMutex);
        if (!_pairingInterface.empty()) peer->setInterface(nullptr, _pairingInterface);
      }
...
std::shared_ptr<Variable> MAXCentral::setInstallMode(BaseLib::PRpcClientInfo clientInfo, bool on, uint32_t duration, BaseLib::PVariable metadata, bool debugOutput) {
  try {
    std::unique_lock<std::mutex> pairingModeThreadGuard(_pairingModeThreadMutex, std::defer_lock);
    std::unique_lock<std::mutex> pairingInterfaceGuard(_pairingInterfaceMutex, std::defer_lock);
    std::lock(pairingModeThreadGuard, pairingInterfaceGuard);
    if (_disposing) {
