...
#include "MAXSpyDevice.h"
#include "../GD.h"

namespace MAX
{
MAXSpyDevice::MAXSpyDevice(IDeviceEventSink* eventHandler) : MAXDevice(eventHandler)
{
	init();
}

MAXSpyDevice::MAXSpyDevice(uint32_t deviceID, std::string serialNumber, int32_t address, IDeviceEventSink* eventHandler) : MAXDevice(deviceID, serialNumber, address, eventHandler)
{
	init();
}

MAXSpyDevice::~MAXSpyDevice()
{
	dispose();
}

void MAXSpyDevice::init()
{
	try
	{
		MAXDevice::init();
...
void MAXSpyDevice::saveVariables()
{
	try
	{
		if(_deviceID == 0) return;
		MAXDevice::saveVariables();
...
void MAXSpyDevice::loadVariables()
{
	try
	{
		MAXDevice::loadVariables();
...
void MAXSpyDevice::serializeFilters(std::vector<uint8_t>& encodedData)
{
	try
	{
		BaseLib::BinaryEncoder encoder(_bl);
		encoder.encodeInteger(encodedData, _filters.size());
		for(std::list<MAXSpyDevice_Filter>::const_iterator i = _filters.begin(); i != _filters.end(); ++i)
		{
...
void MAXSpyDevice::unserializeFilters(std::shared_ptr<std::vector<char>> serializedData)
{
	try
	{
...
bool MAXSpyDevice::onPacketReceived(std::string& senderID, std::shared_ptr<BaseLib::Systems::Packet> packet)
{
	try
	{
		...
		for(std::list<MAXSpyDevice_Filter>::const_iterator i = _filters.begin(); i != _filters.end(); ++i)
		{
			switch(i->filterType)
			{
				case FilterType::SenderAddress:
					if(maxPacket->senderAddress() == i->filterValue) printPacket = true;
					break;
				case FilterType::DestinationAddress:
					if(maxPacket->destinationAddress() == i->filterValue) printPacket = true;
					break;
			}
		}
...
void MAXSpyDevice::addFilter(FilterType filterType, int32_t filterValue)
{
	try
	{
		MAXSpyDevice_Filter filter;
...
void MAXSpyDevice::removeFilter(FilterType filterType, int32_t filterValue)
{
	try
	{
		_filters.remove_if([&](MAXSpyDevice_Filter filter){ return filter.filterType == filterType && filter.filterValue == filterValue; });
	}
    catch(const std::exception& ex)
    {
    	GD::out.printEx(__FILE__, __LINE__, __PRETTY_FUNCTION__, ex.what());
...
std::string MAXSpyDevice::handleCLICommand(std::string command)
{
	try
	{
		...
		if(command == "help")
		{
			...
			stringStream << "send\t\t\tSends a MAX packet" << std::endl;
		...
		if(command.compare(0, 12, "filters list") == 0)
		{
			...
			while(std::getline(stream, element, ' '))
			{
				...
				else if(index == 2)
				{
					if(element == "help")
					{
						stringStream << "Description: This command lists all MAX packet filters." << std::endl;
		...
		else if(command.compare(0, 11, "filters add") == 0)
		{
			...
			if(index < 4)
			{
				stringStream << "Description: This command adds a MAX packet filter. All filters are linked by \"or\". Currently there is no option to connect the filters by \"and\"" << std::endl;
		...
		else if(command.compare(0, 14, "filters remove") == 0)
		{
			...
			if(index < 4)
			{
				stringStream << "Description: This command removes a MAX packet filter." << std::endl;
		...
		else if(command.compare(0, 4, "send") == 0)
		{
			...
			if(index == 1)
			{
				stringStream << "Description: Sends a MAX packet." << std::endl;
